...
- `public/overlays/clock.html` を単体HTMLとして配布する。基準640×144px、外側は透過、内側は第3章の `--bg` による不透明パネル。角を斜めに切り、どのスライド背景でも文字のコントラストを保つ。
- 第4章のBarlow Condensed SemiBoldを埋め込み、等幅数字で横揺れを防ぐ。ブランドと日付を左、時刻を中央右、回転する星のリングを右端に置く。
- 日付は `YYYY.MM.DD`、時刻は `HH:mm:ss`。JST固定、24時間制。時・分・秒の数字はすべてクリームで統一する。星と回転リング上の点、下辺のグラデーションにゴールドを使い、コロンとブランドはライム、回転リングにはラベンダーを添える。
- 今回のユーザー指定により第17章の常時点滅回避の例外として、コロンのみ1秒周期で不透明度を1〜0.28の範囲で変化させる。数字は常時表示する。
- 125 BPMを基準にインジケーターを2拍周期で脈動させる。リングとグラデーションは8拍相当の3.84秒周期、背景光は16拍相当の7.68秒で往復。BGM音声には同期せず、音声再生は追加しない。
- 第22章の低減設定を尊重しつつ、OBS向けに `motion=full` の明示指定を設ける。静止設定でも時計は毎秒更新する。
- 第33章の静的スライドとは独立したブラウザソースとして使用する。配置例と仕様詳細は `docs/stream-clock.md` に記録する。
